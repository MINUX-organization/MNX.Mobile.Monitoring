...
@Composable
internal fun Gpus(
    gpus: List<GpuItemModel>,
    onSettingsClick: (gpuId: String, gpuName: String) -> Unit,
    modifier: Modifier = Modifier
) {
    LazyColumn(modifier = modifier) {
        items(gpus, key = { it.id }) {
            GpuItem(
                model = it,
                onSettingsClick = {
                    onSettingsClick(it.id, it.summary.name.deviceName ?: "N/A")
                }
            )
        }
    }
...
@Composable
internal fun GpusError(modifier: Modifier = Modifier) {
    Column(
